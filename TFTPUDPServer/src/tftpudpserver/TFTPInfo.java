 ...
 * @author Jon
 */
public class TFTPInfo extends Thread {
    ...
    public TFTPInfo(String name) {
        super(name);
    }
    
    byte[] createReadOrWriteTFTPHeader(byte opCode, String file) {
     ...
     byte[] createDataOrAckTFTPHeader(byte opCode, int block, byte[] data) {
        byte[] tftpHeader;
        if(opCode == 3) {
